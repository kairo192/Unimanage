...
export default function RoomsPage() {
  ...
  const handleDeleteClick = (e, id) => {
    e.stopPropagation();
    setConfirmDialog({
      message: 'Are you sure you want to delete this room? This might affect students currently assigned to it.',
      onConfirm: async () => {
        try {
          ...
          setAlertDialog({ message: 'Failed to delete room.' });
        }
      }
    });
  };
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          setAlertDialog({ message: "Failed to unassign student" });
        }
      }
    });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '800', color: '#1a1a14', marginBottom: '4px' }}>Campus Rooms</h1>
          <p style={{ color: '#8a7f72', fontSize: '13px' }}>Manage room capacities and student assignments</p>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <StatChip label="Available" value={available} color="#5c651f" bg="#f4f6ec" border="#c7d6a2" />
          <StatChip label="Full" value={full} color="#b84a2e" bg="#fce4db" border="#f8c7b4" />
      ...
      {loading ? (
        <div style={{ padding: '48px', textAlign: 'center', color: '#8a7f72' }}>Loading rooms...</div>
        ...
        Object.entries(grouped).map(([bloc, blocRooms]) => (
          <div key={bloc}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              ...
              <span style={{ fontSize: '12px', color: '#8a7f72' }}>· {blocRooms.length} rooms</span>
            </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        {(r.tickets && r.tickets.length > 0) && (
                          <div style={{ width: '22px', height: '22px', borderRadius: '6px', background: '#fce4db', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title={`${r.tickets.length} Active Ticket(s)`}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        ...
                        <button onClick={(e) => handleEditClick(e, r)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8ea45c', padding: '2px' }} title="Edit">
                          <Edit2 size={14} />
                        </button>
                        <button onClick={(e) => handleDeleteClick(e, r.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d45c3c', padding: '2px' }} title="Delete">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
          ...
          <div style={{
            ...
            <div style={{ background: '#f9f7f3', padding: '24px', borderBottom: '1px solid #e8e2d6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <h2 style={{ fontSize: '20px', fontWeight: '800', color: '#1a1a14' }}>Bloc {selectedRoom.building} - {selectedRoom.room_number}</h2>
                  <p style={{ fontSize: '13px', color: '#8a7f72', fontWeight: '600' }}>{selectedRoom.occupied_count} / {selectedRoom.capacity} Beds Occupied</p>
                </div>
              </div>
            ...
            <div style={{ padding: '24px' }}>
              <h3 style={{ fontSize: '13px', fontWeight: '800', color: '#5c651f', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '16px' }}>Current Residents</h3>
                    ...
                    <div key={student.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#fff', border: '1px solid #e8e2d6', borderRadius: '12px', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: '#8a7f72', marginTop: '2px' }}>
                          {student.study_year || 'Unknown Level'} • {student.speciality || 'General'}
                        </div>
                      </div>
                      <button 
              ...
              <h3 style={{ fontSize: '13px', fontWeight: '800', color: '#b84a2e', textTransform: 'uppercase', letterSpacing: '0.5px', marginTop: '24px', marginBottom: '16px' }}>Maintenance Tickets</h3>
              ...
              {(!selectedRoom.tickets || selectedRoom.tickets.length === 0) ? (
                <div style={{ textAlign: 'center', padding: '20px', color: '#8a7f72', background: '#fafaf7', borderRadius: '12px', border: '1px dashed #e8e2d6' }}>
                  No active maintenance tickets for this room.
                </div>
              ) : (
          ...
          <div style={{
            ...
            <div style={{ background: '#f9f7f3', padding: '20px 24px', borderBottom: '1px solid #e8e2d6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <h2 style={{ fontSize: '16px', fontWeight: '800', color: '#1a1a14' }}>{editRoomId ? 'Edit Room' : 'Add New Room'}</h2>
                </div>
              </div>
            ...
            <div style={{ padding: '24px' }}>
              ...
              <form onSubmit={handleSaveRoom} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  ...
                  <input required name="building" value={formData.building} onChange={handleInputChange} style={inputStyle} placeholder="e.g. A, B, Main" />
                </div>
                <div>
                  <label style={labelStyle}>Room Number</label>
                  <input required name="room_number" value={formData.room_number} onChange={handleInputChange} style={inputStyle} placeholder="e.g. 101" />
                </div>
                <div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '16px' }}>
                  ...
                  <button type="submit" disabled={isSubmitting} style={{
                    ...
                    <Save size={16} /> {isSubmitting ? 'Saving...' : (editRoomId ? 'Update Room' : 'Save Room')}
                  </button>
                </div>
              </form>
            </div>
